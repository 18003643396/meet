...
function Uploader(options) {
    ...
    var defaults = {
         ...
         * @type {String}
         */
        urlDns: 'https://wanproxy.127.net/lbs',
        /**
         * 上传输入框元素ID
    ...
    service = {
        ...
        uploadTrunk: function(param, trunkData, callback) {
            ...
            xhr.onreadystatechange = function() {
                ...
                if (xhr.status === 200) {
                    ...
                    service.clearStorage(trunkData.fileKey);
                    //取消、关闭情况
                    opts.onError(xhr.responseText, curFile);
                }
            };
            ...
            xhr.open('post', 'https://nosup-hz1.127.net' + '/' + param.bucketName + '/' + param.objectName + xhrParam);
        ...
        getOffset: function(param, callback) {
            ...
            curFile = service.getFile(param.fileKey);
            if (!context) {
                return callback(0);
            }
            $.ajax({
                type: 'get',
                url: 'https://nosup-hz1.127.net' + '/' + param.bucketName + '/' + param.objectName + '?uploadContext',
                data: {
                    version: '1.0',
                    context: context
                },
        ...
        initEvent: function() {
            $('#' + opts.fileInputId).on('change', function(e) {
                ...
                if (e.target.files && e.target.files.length > 0) {
                	...
                	var curFile = service.findFile(_file);

                	//视频小于等于300M
                	if(_file.size>300*1024*1024){
                		opts.onExceedMaxSize();
                    	return;
                	}
