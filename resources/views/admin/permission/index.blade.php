            ...
            <div class="box-body table-responsive no-padding">
				<table class="table table-hover">
					...
					<tr>
						...
						<td>{{$v->id}}</td>
						<td class='name'>{{$v->per_name}}</td>

						<td>{{$v->per_url}}</td>
						
						<td>
							<a href="/admin/permission/{{$v->id}}/edit"class='btn btn-block btn-default btn-xs'style='width: 50px;'><i class='fa fa-pencil-square-o'></i></a>&nbsp;
...
<script>
	...
	$('#alldelete').click(function(){
			...
			if(confirm("确定要删除所选项目？")) { 
				...
				$.get('/admin/perdeleteajax',{ids:checkedList.toString()},function(data){
