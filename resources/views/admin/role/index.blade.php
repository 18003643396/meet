            ...
            <div class="box-body table-responsive no-padding">
				<table class="table table-hover">
					...
					<tr>
						...
						<td>{{$v->id}}</td>
						<td class='name'>{{$v->role_name}}</td>
						<td><a href="/admin/role_per?id={{$v->id}}"class='btn btn-block btn-default btn-xs'style='width: 40px;'><i class='fa fa-gear'></i></a></td>
						<td>
							<a href="/admin/role/{{$v->id}}/edit"class='btn btn-block btn-default btn-xs'style='width: 50px;'><i class='fa fa-pencil-square-o'></i></a>&nbsp;
...
<script>
	...
	$('#alldelete').click(function(){
			...
			if(confirm("确定要删除所选项目？")) { 
				...
				$.get('/admin/roledeleteajax',{ids:checkedList.toString()},function(data){
