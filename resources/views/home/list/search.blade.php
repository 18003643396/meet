    ...
    <div class="container">
       ...
       <div class="ajax-load-box posts-con">
        @foreach($ainfo as $k => $v)
            <div class="ajax-load-con content wow fadeInUp"><div class="content-box posts-image-box"><div class="posts-default-title"><h2><a href="/home/xiangqing?id={{$v->id}}" title="" target="_blank">{{$v->title}}</a></h2></div><div class="posts-default-content"><div class="posts-text">{{strip_tags( str_limit($v->content,200) )}}&hellip;</div></div><div class="posts-default-info"><ul><li class="post-author hidden-xs"><div class="avatar"><img alt="maolai" src="{{$v->img}}" height="96" width="96"/></div><a href="javascript:void(0)">{{$v->name}}</a></li><li class="ico-cat"><i class="icon-list-2"></i><a href="#">wordpress主题</a></li><li class="ico-time"><i class="icon-clock-1"></i>{{$v->time}}</li><li class="ico-eye hidden-xs"><i class="icon-eye-4"></i> {{$v->count}}</li><li class="ico-like hidden-xs"><i class="icon-thumbs-up"></i> {{$v->zan}}</li></ul></div></div></div>
        @endforeach
           </div> 
